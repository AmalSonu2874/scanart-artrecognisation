...
const Index = () => {
  ...
  const handleCommand = (command: string): string => {
    const commands: Record<string, string | (() => string)> = {
      ...
      reset: () => {
        handleClearImage();
        return "Upload and prediction cleared.";
      },
      theme: () => {
        toggleTheme();
        return `Theme switched to ${!isDark ? "dark" : "light"} mode.`;
      },
      ...
      history: () => {
        ...
        return history.slice(0, 5).map((h: {
          label: string;
          confidence: number;
        }) => `${h.label} (${Math.round(h.confidence * 100)}%)`).join("\n");
      }
    };
  ...
  return <div className="min-h-screen grid-bg">
      ...
      <main className="container mx-auto px-4 pt-28 pb-8 border-dashed shadow-md rounded-md">
        ...
        <section className="max-w-2xl mx-auto mb-12">
          ...
          {currentImage && <div className="flex flex-wrap gap-3 mt-6 justify-center">
              ...
              {prediction && <Button variant="secondary" onClick={shareResult} className="hard-shadow-sm font-mono">
          ...
          <div className="mt-6">
            <PredictionResult prediction={prediction} isLoading={isAnalyzing} onRerun={runAnalysis} />
          </div>
        </section>
